...
const requireNotificationAccess = () => {
  return (req, res, next) => {
    ...
    // Admin can access all notification data
    if (["admin", "super_admin"].includes(req.user.role)) {
      return next();
    }
    ...
    logger.warn("Access denied - notification data access restricted:", {
      userId: req.user._id,
      targetUserId,
      requestId: req.requestId,
    });

    return next(new ForbiddenError("Access denied - can only access own notifications"));
  };
};
