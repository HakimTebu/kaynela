...
const requirePaymentAccess = () => {
  return (req, res, next) => {
    ...
    logger.warn("Access denied - payment data access restricted:", {
      userId: req.user._id,
      targetUserId,
      requestId: req.requestId,
    });

    return next(new ForbiddenError("Access denied - can only access own payment data"));
  };
};
