...
// Check if user can access loyalty data
const requireLoyaltyAccess = () => {
  return (req, res, next) => {
    if (!req.user) {
      return next(new UnauthorizedError("Authentication required"));
    }

    // Admin can access all loyalty data
    if (["admin", "super_admin"].includes(req.user.role)) {
      return next();
    }
    ...
    logger.warn("Access denied - loyalty data access restricted:", {
      userId: req.user._id,
      targetUserId,
      requestId: req.requestId,
    });

    return next(new ForbiddenError("Access denied - can only access own loyalty data"));
  };
};
