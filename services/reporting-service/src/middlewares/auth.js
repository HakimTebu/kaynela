...
// Check if user can access reporting data
const requireReportingAccess = () => {
  return (req, res, next) => {
    ...
    // Admin can access all reporting data
    if (["admin", "super_admin"].includes(req.user.role)) {
      return next();
    }
    ...
    logger.warn("Access denied - reporting data access restricted:", {
      userId: req.user._id,
      targetUserId,
      requestId: req.requestId,
    });

    return next(new ForbiddenError("Access denied - can only access own reporting data"));
  };
};
